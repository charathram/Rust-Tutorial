...
impl<'a> Report<'a> {
    ...
    ///
    /// ```
    /// use rwc::{Counts, Report};
    /// let report = Report::new("-", Counts::from_text("a b c\n"));
    /// assert_eq!(report.line(), "     1      3      6      6 -");
    /// ```
    pub fn line(&self) -> String {
        format!(
            "{:>6} {:>6} {:>6} {:>6} {}",
            self.counts.lines, self.counts.words, self.counts.bytes, self.counts.chars, self.label
        )
    }
}
...
/// `chars` counts Unicode scalar values (`char`).
///
/// ```
/// use rwc::Counts;
...
mod tests {
    ...
    fn report_borrows_label_and_holds_counts() {
        ...
        assert_eq!(report.line(), "     1      3      6      6 demo.txt");
    }
}
