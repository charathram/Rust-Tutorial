...
- **The glossary (`reference/glossary.html`) is kept in alphabetical order** by term — case-insensitive, ignoring a leading "The"; symbol-led terms (e.g. the `?` operator) sort first. When adding a term, insert it in its correct slot, not at the end. (Stated by user after Lesson 6.)
...
- **Lesson 4 · Borrowing & references** ✅ — `0004-borrowing.html`, project `0004-rwc/`. Feature driver: rwc counts *several* files + prints a grand total. Teaches shared `&` (recap `&text`; `&paths` so the Vec survives the loop), mutable `&mut` (a `add_into(&mut total, …)` accumulator), and the shared-XOR-mutable rule (+ no-dangling → lifetimes L9). C/Go contrast. Lesson 3 tests stay green as the refactor safety net; added an `add_into` test. std-only. **Learner completed it** (their finished `0004-rwc/main.rs` = canonical Step-2 version + `add_into_accumulates` test).
- **Lesson 5 · Reading borrow-checker errors** ✅ AUTHORED — `0005-borrow-checker-errors.html`, project `0005-rwc/` (clean copy of finished L4, clippy-clean baseline). A *diagnostics* lesson, not build-along: learner deliberately breaks rwc to provoke four classic errors, reads each structurally, restores baseline. Covers **E0596** (forgot `mut` → E0596 not E0384, since the trigger is `&mut` of an immutable binding), **E0382** (drop `&` in `for path in paths` → use-after-move via implicit `.into_iter()`), **E0502** (peek `&total` across `&mut total`), **E0499** (two `&mut`). Teaches error *anatomy* (code/summary/spans/help) + NLL ("borrow lives to last use") + `rustc --explain`/Error Index. **All four error outputs captured from real `rustc` 1.96 against the actual file so line numbers (35/29/23/30/37/36/38) match what the learner sees.** std-only. Verified baseline: 4 tests pass, runs, clippy clean.
- **Lesson 6 · Structs & enums** ✅ AUTHORED — `0006-structs-enums.html`, project `0006-rwc/`. Build-along. `Counts` struct (`#[derive(Debug,Default,Clone,Copy,PartialEq,Eq)]`, `from_text`/`add` methods) replaces the `(usize,usize,usize)` tuple; `Source` enum (`Stdin`/`File(String)`, `from_args`/`name`/`read`) — **delivers stdin** (`echo x | rwc`), the module-checkpoint stdin requirement, early. Logic moved to **`src/lib.rs`** (lib+bin split, `use rwc::{Counts,Source}`) → **doc-tests land here** as promised: `///` examples, hidden `#` setup lines, fence annotations (`no_run` used for real on `Source::read`; `ignore`/`should_panic`/`compile_fail` in a concept box). All output real: 6 unit + 5 doc-tests pass, clippy clean, both file & stdin paths run. **L6 uses a minimal 2-arm `match` in `name`/`read`** (introduced with a one-line forward-ref) — so **L7 was re-anchored**: from "do the file/stdin dispatch" → "the same `match` in *depth* — exhaustiveness as a compile guarantee (add a variant, watch matches fail), guards, bindings, `if let`/`let else`." Propagated to nav.js + syllabus L7 desc. **Glossary += struct, enum, derive, library-vs-binary crate, doc-test.**
- **Lesson 7 · Pattern matching with `match`** ← NEXT — deepen the `Source` `match` (see re-anchor above). Project likely `0007-rwc/`. (Module 2 batch continues: L8 strings/slices, L9 lifetimes.)
