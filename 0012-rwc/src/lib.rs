...
/// Everything that can go wrong inside the `rwc` library.
#[derive(Debug, Error)]
pub enum RwcError {
    /// An argument began with a dash, but was not a recognized flag.
    #[error("invalid argument: {0}")]
    InvalidArgument(String),

    /// An IO error occurred while reading a file or standard input.
    #[error(transparent)]
    Io(#[from] io::Error),
}
...
impl Source {
    ...
    /// ```
    pub fn from_args(args: Vec<String>) -> Result<Vec<Source>, RwcError> {
        if args.is_empty() {
        ...
        args.into_iter()
            .map(|arg| match arg.as_str() {
                "-" => Ok(Source::Stdin),
                flag if flag.starts_with('-') => Err(RwcError::InvalidArgument(flag.to_string())),
    ...
    /// # Ok::<(), rwc::RwcError>(())
    /// ```
    pub fn read(&self) -> Result<String, RwcError> {
        match self {
...
mod tests {
    ...
    fn invalid_argument_error_message() {
        let err = RwcError::InvalidArgument("-x".to_string());
        assert_eq!(err.to_string(), "invalid argument: -x");
    }
}
